...
# Nota: debes definir o importar las funciones auxiliares _ritmo_to_minutos y _ensure_fecha_datetime que usas en los cálculos.


# ====================================================================
def tab_histograma_ritmos(df):
    """Histograma de ritmos con distribución normal, como en el ZIP."""
    if df.empty or "Ritmos" not in df.columns:
...
def tab_mejores_sesiones_ritmo_distancia(df):
    """
    Ritmo vs Distancia mostrando las 5 mejores sesiones (curvas),
    agrupadas por Fecha como en el ZIP original, adaptado a Streamlit.
    """
    ...
    p = figure(
        title="Mejores Sesiones (Ritmo vs Distancia)",
...
def tab_ritmo_medio_fecha(df):
    """Ritmo medio por fecha (línea y área bajo la línea, eje Y ajustado)"""
...
def tab_tabla_por_fecha(df):
    """Tabla con distancia máxima y ritmo promedio como mm:ss por fecha."""
    if df.empty:
        # Mostrar mensaje en Streamlit
        st.warning("No hay datos disponibles.")
        # Devolver objeto Div para mostrar y HTML vacío para descarga
    ...
    # Generar HTML para descargar con pandas
    # Seleccionamos columnas para el reporte HTML
    agg_html = agg[["Fecha_str", "Distancia_km", "Ritmo_mmss"]].rename(
        columns={"Fecha_str": "Fecha", "Distancia_km": "Distancia total (km)", "Ritmo_mmss": "Ritmo Promedio (mm:ss)"}
    )
...
def tab_barras_lugares(df):
    if df.empty or "Lugar" not in df.columns or "Periodo" not in df.columns:
        return figure(title="No hay datos de lugares o periodos")
    ...
    # Calcular máximo total para ajustar rango X
    max_total = (pivot.sum(axis=1)).max()
    x_range_max = max_total + 5  # Se agrega 5 unidades más

    # Aumentar altura base
    height_px = 90 + 35 * len(lugares_ordenados)
    ...
    p = figure(
        ...
        y_axis_label="Lugar",
        title="Kilómetros por Lugar y Periodo",
...
def tab_data_completo(df):
    """Tabla con anchos ajustados para ID y Lugar, y formato pedido."""
    if df.empty:
